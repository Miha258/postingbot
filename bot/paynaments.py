    ...
    async with aiohttp.ClientSession(headers = headers) as session:
        async with session.get(f'https://pay.crypt.bot/api/getInvoices?invoice_id={check_id}') as response:
            ...
            if paynament['status'] != 'paid':
                ...
                await Bots.update("id", callback_query.message.from_id, subscription = True, subscription_to = datetime.now() + timedelta(days = 31))
...
def register_paynaments(dp: Dispatcher):
    dp.register_message_handler(choose_tarif, text = "Тарифи", state = "*")
