...
async def option_handler(callback_query: types.CallbackQuery):
    ...
    if type == 'bot_checking':
        ...
        if capcha is None:
            capcha = (await Channels.update( _channel['id'], channel, capcha = "Пройдіть перевірку на бота - Відмовитися | Ви відмовилися від перевірки , Підтвердити | Перевірку пройдено"))['capcha']
